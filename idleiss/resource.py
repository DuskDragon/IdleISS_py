...
class ResourceManager(object):
    def __init__(self, savedata=None):
        ...
        self.structures = savedata['structures']
...
\tsources: {self.structures}"""
    ...
    def add_structure(self, system_name, structure_name, basic_income,
        ...
        if system_name in self.structures:
            if structure_name in self.structures[system_name]:
                raise Location_Already_Exists(f"{structure_name}@{system_name} already exists.")
            else:
                self.structures[system_name].update({structure_name: [basic_income, adv_income, money_income]})
        else:
            self.structures.update({system_name: {structure_name: [basic_income, adv_income, money_income]}})

    def remove_structure(self, system_name, structure_name):
        if system_name in self.structures:
            if structure_name in self.structures[system_name]:
                b_i, a_i, m_i = self.structures[system_name][structure_name]
                ...
                self.structures[system_name].pop(structure_name)
                if len(self.structures[system_name]) == 0:
                    self.structures.pop(system_name)
            else:
                raise Location_Does_Not_Exist(f"{structure_name}@{system_name} does not exist.")
        else:
            raise Location_Does_Not_Exist(f"{structure_name}@{system_name} does not exist.")

    def update_structure(self, system_name, structure_name, basic_income, adv_income, money_income):
        if basic_income < 0 or adv_income < 0 or money_income < 0:
            raise ValueError("Income for an income source cannot be negative")
