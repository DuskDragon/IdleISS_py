...
class Battle(object):
    ...
    def generate_summary_text(self):
        ...
        for attacking_ship in attacker_ships:
            out_array.append(f"    {attacking_ship}: {summary['attacker_fleet'][attacking_ship]}\n")
        out_array.append("Defender:\n")
        defender_fleet = summary["defender_fleet"]
        if len(defender_fleet) == 0:
            out_array.append("    NO DEFENDING SHIPS\n")
        else:
            defender_ships = []
            ...
            for defending_ship in defender_ships:
                out_array.append(f"    {defending_ship}: {summary['defender_fleet'][defending_ship]}\n")
        out_array.append("\nResult:\nAttacker:\n")
        attacker_fleet_result = summary["attacker_result"]
        if len(attacker_fleet_result) == 0:
            out_array.append("    ALL ATTACKING SHIPS DESTROYED\n")
        else:
            attacker_result = []
            ...
            for attacking_ship in attacker_result:
                out_array.append(f"    {attacking_ship}: {attacker_fleet_result.get(attacking_ship,0)} (Lost: {summary['attacker_losses'][attacking_ship]})\n")
        out_array.append("Defender:\n")
        defender_fleet_result = summary["defender_result"]
        if len(defender_fleet_result) == 0:
            out_array.append("    ALL DEFENDING SHIPS DESTROYED")
        else:
            defender_result = []
            ...
            for defending_ship in defender_result:
                out_array.append(f"    {defending_ship}: {defender_fleet_result.get(defending_ship,0)} (Lost: {summary['defender_losses'][defending_ship]})\n")
        output_str = "".join(out_array)
